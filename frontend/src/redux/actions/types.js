// User REducer
export const LOG_IN_REQUEST = 'LOG_IN_REQUEST';
export const LOG_IN_SUCCESS = 'LOG_IN_SUCCESS';
export const LOG_IN_FAILURE = 'LOG_IN_FAILURE';

export const LOG_OUT_REQUEST = 'LOG_OUT_REQUEST';
export const LOG_OUT_SUCCESS = 'LOG_OUT_SUCCESS';
export const LOG_OUT_FAILURE = 'LOG_OUT_FAILURE';

export const AUTO_LOG_IN = 'AUTO_LOG_IN';

// Court Info Reducer
export const SELECT_COURT = 'SELECT_COURT';
export const UNSELECT_COURT = 'UNSELECT_COURT';
export const REMOVE_SELECT_COURT = 'REMOVE_SELECT_COURT';
export const MOUSEOVER_LIST = 'MOUSEOVER_LIST';
export const GET_FILTER_INPUT = 'GET_FILTER_INPUT';
export const GET_SEARCH_INPUT = 'GET_SEARCH_INPUT';
export const INIT_FILTER_INPUT = 'INIT_FILTER_INPUT';

export const SEARCH_COURTS_REQUEST = 'SEARCH_COURTS_REQUEST';
export const SEARCH_COURTS_SUCCESS = 'SEARCH_COURTS_SUCCESS';
export const SEARCH_COURTS_FAILURE = 'SEARCH_COURTS_FAILURE';

export const SEARCH_COURT_REQUEST = 'SEARCH_COURT_REQUEST';
export const SEARCH_COURT_SUCCESS = 'SEARCH_COURT_SUCCESS';
export const SEARCH_COURT_FAILURE = 'SEARCH_COURT_FAILURE';

// UI Controlling Reducer
export const TOGGLE_SIDEBAR = 'TOGGLE_SIDEBAR';
export const OPEN_NICKNAME_CHANGER = 'OPEN_NICKNAME_CHANGER';
export const CLOSE_NICKNAME_CHANGER = 'CLOSE_NICKNAME_CHANGER';
export const TOGGLE_USER_MENU = 'TOGGLE_USER_MENU';
export const CLOSE_USER_MENU = 'CLOSE_USER_MENU';
export const OPEN_DELETE_REVIEW_MODAL = 'OPEN_DELETE_REVIEW_MODAL';
export const CLOSE_DELETE_REVIEW_MODAL = 'CLOSE_DELETE_REVIEW_MODAL';
export const OPEN_ADD_REVIEW_FORM = 'OPEN_ADD_REVIEW_FORM';
export const CLOSE_ADD_REVIEW_FORM = 'CLOSE_ADD_REVIEW_FORM';
export const NO_DATA = '정보없음';

// Review Reducer
export const LOAD_ALLREVIEWS_REQUEST = 'LOAD_ALLREVIEWS_REQUEST';
export const LOAD_ALLREVIEWS_SUCCESS = 'LOAD_ALLREVIEWS_SUCCESS';
export const LOAD_ALLREVIEWS_FAILURE = 'LOAD_ALLREVIEWS_FAILURE';

export const LOAD_MYREVIEW_REQUEST = 'LOAD_MYREVIEW_REQUEST';
export const LOAD_MYREVIEW_SUCCESS = 'LOAD_MYREVIEW_SUCCESS';
export const LOAD_MYREVIEW_FAILURE = 'LOAD_MYREVIEW_FAILURE';

export const UPLOAD_REVIEW_REQUEST = 'UPLOAD_REVIEW_REQUEST';
export const UPLOAD_REVIEW_SUCCESS = 'UPLOAD_REVIEW_SUCCESS';
export const UPLOAD_REVIEW_FAILURE = 'UPLOAD_REVIEW_FAILURE';

export const DELETE_MYREVIEW_REQUEST = 'DELETE_MYREVIEW_REQUEST';
export const DELETE_MYREVIEW_SUCCESS = 'DELETE_MYREVIEW_SUCCESS';
export const DELETE_MYREVIEW_FAILURE = 'DELETE_MYREVIEW_FAILURE';

export const EMPTY_REVIEWS = 'EMPTY_REVIEWS';